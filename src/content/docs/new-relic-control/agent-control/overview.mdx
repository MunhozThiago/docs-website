...
Agent Control is designed to be a supervisor for managing instrumentation, which requires the ability to perform powerful actions within your environment. It's critical to understand and grant the required permissions before deploying.
...
### Linux hosts
For Linux and Windows hosts (currently in public preview), Agent Control requires elevated permissions. The supervisor needs to run with root, administrator, or equivalent privileges to perform agent lifecycle tasks such as:
