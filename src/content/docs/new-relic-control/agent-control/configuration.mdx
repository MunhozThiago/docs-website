...
Agent Control manages configuration at two distinct levels, and understanding the difference determines where you make changes:

**Configure Agent Control itself:** Connection details (Fleet Control endpoint, OpAMP credentials), system identity, fleet association, and proxy settings. These are set at installation time in your local configuration file and rarely need to change. They control how the supervisor operates, not what it deploys.

**Configure managed agents through Agent Control:** Agent-specific settings — versions, log paths, resource limits, scrape intervals, integration toggles — are managed in Fleet Control's remote configuration. Agent Control receives these settings over the OpAMP channel and applies them to each agent. Change a setting in Fleet Control; it propagates to every cluster and host in your fleet automatically.

This distinction matters when you're troubleshooting: if Agent Control isn't connecting to Fleet Control, look at the local config. If an agent isn't behaving as expected, look at Fleet Control's remote config.

## Two layers, one file

Agent Control's configuration uses two layers, which can coexist in the same file:

1. **Agent Control's core configuration:** Top-level settings that control how the supervisor itself operates — connection to New Relic, identity, fleet association.

2. **Managed agents' configuration:** The `chart_values` or `agent_values` for each agent Agent Control deploys and manages (for example, Infrastructure Agent scrape settings, Fluent Bit log paths).

When both a local and a remote configuration are present, Agent Control follows this precedence rule:

- **Remote configuration always wins.** Any settings from Fleet Control override the corresponding settings in the local `values.yaml` or `config.yaml`.
- To intentionally revert to local settings, deploy an empty remote configuration in Fleet Control. The change applies to all clusters and hosts in the selected fleet.
...
You can also use environment variables to override agent settings:
* Use the `NR_` (single underscore) prefix to target the main Agent Control configuration.
* Use `__` (double underscores) to separate nested keys. This is required to avoid collisions with configuration keys that contain underscores.
* Environment variables take precedence over the local configuration file. If remote configuration is enabled, environment variables are not considered.
* For example, to set the `fleet_control.endpoint` dynamically: `NR_FLEET_CONTROL__ENDPOINT=https://opamp.service.newrelic.com/v1/opamp`
